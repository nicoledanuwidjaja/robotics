...
void fix_guess(Robot* robot) {
  ...
  for (int i1 = -range; i1 <= range; ++i1)
  {
    for (int i2 = -range; i2 <= range; ++i2)
    {
      ...
      float cum_err = abs((guess[0] + i1 * grid_div) - (last_guess[0] + c_col[0])) * predict_weight
          + abs((guess[1] + i2 * grid_div) - (last_guess[1] + c_col[1])) * predict_weight + err - (hit * hit_weight);
